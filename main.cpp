...
bool StringToInt(const std::wstring& s, int& value) {
    try {
        ...
        if (idx != s.size()) return false; // trailing junk
        value = v;
        return true;
    }
    catch (...) {
...
bool ParseDiceNotation(const std::wstring& notation,
    int& numDice,
    int& sides,
    int& modifier)
{
    ...
    if (plusPos != std::wstring::npos) modPos = plusPos;
    else if (minusPos != std::wstring::npos) modPos = minusPos;
...
void SetupControls(HWND hWnd) {
    // CSV File Name
    CreateWindowW(L"STATIC", L"CSV File:",
        WS_VISIBLE | WS_CHILD,
        20, 10, 80, 20,
        hWnd, nullptr, nullptr, nullptr);

    CreateWindowW(L"EDIT", L"dice_results.csv",
        WS_VISIBLE | WS_CHILD | WS_BORDER | ES_AUTOHSCROLL,
        110, 10, 250, 20,
        hWnd, (HMENU)IDC_FILE_EDIT, nullptr, nullptr);
    ...
    // Dice notation
    CreateWindowW(L"STATIC", L"Dice (e.g. 2d6+3):",
        WS_VISIBLE | WS_CHILD,
        20, 70, 130, 20,
        hWnd, nullptr, nullptr, nullptr);

    CreateWindowW(L"EDIT", L"1d100+0",
        WS_VISIBLE | WS_CHILD | WS_BORDER | ES_AUTOHSCROLL,
        160, 70, 100, 20,
        hWnd, (HMENU)IDC_DICE_EDIT, nullptr, nullptr);

    // Times
    CreateWindowW(L"STATIC", L"Times:",
        WS_VISIBLE | WS_CHILD,
        20, 100, 80, 20,
        hWnd, nullptr, nullptr, nullptr);

    CreateWindowW(L"EDIT", L"1",
        WS_VISIBLE | WS_CHILD | WS_BORDER | ES_AUTOHSCROLL,
        110, 100, 80, 20,
        hWnd, (HMENU)IDC_TIMES_EDIT, nullptr, nullptr);

    // Mode group
    HWND hGroup = CreateWindowW(L"BUTTON", L"Mode",
    ...
    // Normal
    HWND hRadNormal = CreateWindowW(L"BUTTON", L"Normal",
        WS_VISIBLE | WS_CHILD | BS_RADIOBUTTON | WS_GROUP | WS_TABSTOP,
        30, 150, 100, 20,
        hWnd, (HMENU)IDC_RAD_NORMAL, nullptr, nullptr);

    // Advantage
    CreateWindowW(L"BUTTON", L"Advantage",
        WS_VISIBLE | WS_CHILD | BS_RADIOBUTTON,
        140, 150, 100, 20,
        hWnd, (HMENU)IDC_RAD_ADV, nullptr, nullptr);

    // Disadvantage
    CreateWindowW(L"BUTTON", L"Disadvantage",
        WS_VISIBLE | WS_CHILD | BS_RADIOBUTTON,
        260, 150, 120, 20,
        hWnd, (HMENU)IDC_RAD_DISADV, nullptr, nullptr);

    // Default mode: Normal
    SendMessageW(hRadNormal, BM_SETCHECK, BST_CHECKED, 0);

    // Roll button
    CreateWindowW(L"BUTTON", L"Roll",
        WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
        450, 145, 80, 24,
        hWnd, (HMENU)IDC_ROLL_BUTTON, nullptr, nullptr);

    // Open CSV button
    CreateWindowW(L"BUTTON", L"Open CSV",
        WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON,
        540, 145, 80, 24,
        hWnd, (HMENU)IDC_OPEN_BUTTON, nullptr, nullptr);
...
void HandleRoll(HWND hWnd) {
    ...
    if (diceStrW.empty()) {
        MessageBoxW(hWnd, L"Please enter dice notation like 2d6+3 or d20-1.",
    ...
    if (!StringToInt(GetEditText(hTimes), times) || times <= 0) {
        MessageBoxW(hWnd, L"Times must be a positive integer.",
            L"Input Error", MB_ICONWARNING);
        return;
    }

    int numDice = 0, sides = 0, modifier = 0;
    if (!ParseDiceNotation(diceStrW, numDice, sides, modifier)) {
        MessageBoxW(hWnd, L"Invalid dice notation. Use forms like 2d6+3, d20-1, 3d8.",
            L"Dice Error", MB_ICONWARNING);
        return;
    }
    ...
    if (!out) {
        MessageBoxW(hWnd, L"Failed to open CSV file for writing.",
            L"File Error", MB_ICONERROR);
        return;
    }
    ...
    case MODE_ADV:    modeStr = "Advantage"; break;
    case MODE_DISADV: modeStr = "Disadvantage"; break;
    }
    ...
    for (int i = 0; i < times; ++i) {
        ...
        wchar_t tsW[64];
        wcsftime(tsW, sizeof(tsW) / sizeof(wchar_t), L"%Y-%m-%d %H:%M:%S", &tm_buf);

        char tsA[64];
        std::strftime(tsA, sizeof(tsA), "%Y-%m-%d %H:%M:%S", &tm_buf);
...
void HandleOpenCsv(HWND hWnd) {
    ...
    std::wstring filenameW = GetEditText(hFile);
    if (filenameW.empty()) {
        MessageBoxW(hWnd, L"Please enter or choose a CSV file first.",
    ...
    HINSTANCE res = ShellExecuteW(hWnd, L"open", fullPathW.c_str(),
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDC_RAD_ADV:
        case IDC_RAD_DISADV:
            // Make radios mutually exclusive (simple way)
